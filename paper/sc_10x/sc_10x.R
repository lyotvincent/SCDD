...
scTSSR.path <- paste0("results/scTSSR/sc_10x_scTSSR_impute.tsv")
EnImpute.path <- paste0("results/EnImpute/sc_10x_EnImpute_impute.tsv")
...
scTSSR.st <- sc_10x.st(scTSSR.path, has.name = T)
EnImpute.st <- sc_10x.st(EnImpute.path, has.name = T)
...
scTSSR.umap <- sc_10x.umap(scTSSR.st, "scTSSR")
EnImpute.umap <- sc_10x.umap(EnImpute.st, "EnImpute")


plt <- ggarrange(raw.umap, SCDD.umap, Diffusion.umap,
          ncol = 3, nrow = 1, common.legend=T, legend="none")
ggsave("paper/sc_10x/sc_10x_umap41.eps", plot = plt, width = 6.5, height = 2.4, dpi = 400)
dev.off()
...
ggsave("paper/sc_10x/sc_10x_umap42.eps", plot = plt, width = 12, height = 4.6, dpi = 400)
dev.off()
